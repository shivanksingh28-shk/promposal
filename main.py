...
CONFETTI_COLORS = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 165, 0), (255, 105, 180)]
...
class Puppy:
    ...
    def update(self):
        ...
        if self.state == "BLINKING":
            self.blink_timer -= 1
            if self.blink_timer <= 0: self.state = "IDLE"
        self.vel_y += self.gravity
        self.rect.y += self.vel_y

    def draw(self, surface):
        if self.state == "IDLE" or self.state == "JUMPING": current_img = self.base_open
        else:
        ...
        if self.rect.centery < self.ground_y:
            if self.vel_y < -2: anim_img = pygame.transform.scale(current_img, (110, 145))
            elif self.vel_y > 2: anim_img = pygame.transform.scale(current_img, (135, 115))
            else: anim_img = current_img
...
for _ in range(60): 
    rx, ry = random.randint(0, 99), random.randint(50, 99)
...
f_size = (45, 45)
f1, f2, f3 = pygame.transform.scale(flower1_img, f_size), pygame.transform.scale(flower2_img, f_size), pygame.transform.scale(flower3_img, f_size)
...
for cx, cy in cluster_centers:
    ...
    for _ in range(random.randint(4, 6)):
        rx, ry = cx + random.randint(-25, 25), cy + random.randint(-15, 15)
...
scaled_text1_rect = scaled_text1.get_rect(center=(200, 25))
scaled_text2 = create_pixel_text("if you'd go to prom with me!")
scaled_text2_rect = scaled_text2.get_rect(center=(200, 55))
yay_text = create_pixel_text("YAY! Best Prom Ever!")
yay_rect = yay_text.get_rect(center=(200, 40))
yes_btn, no_btn = pygame.Rect(100, 85, 70, 35), pygame.Rect(230, 85, 70, 35)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if yes_btn.collidepoint(event.pos) and not prom_accepted:
                    ...
                    for _ in range(150):
                        ...
                        vx, vy = random.uniform(-6, 6), random.uniform(-12, -4)
                        confetti_particles.append([cx, cy, vx, vy, random.choice(CONFETTI_COLORS), random.randint(4, 8)])
        
        if not prom_accepted:
            screen.blit(scaled_text1, scaled_text1_rect)
            screen.blit(scaled_text2, scaled_text2_rect)
            pygame.draw.rect(screen, GREEN, yes_btn); pygame.draw.rect(screen, BLACK, yes_btn, 3); screen.blit(yes_text, yes_text_rect)
            pygame.draw.rect(screen, RED, no_btn); pygame.draw.rect(screen, BLACK, no_btn, 3); screen.blit(no_text, no_text_rect)
        else:
            screen.blit(yay_text, yay_rect)
            for p in confetti_particles:
                p[0] += p[2]; p[1] += p[3]; p[3] += 0.4
                pygame.draw.rect(screen, p[4], (p[0], p[1], p[5], p[5]))

        for img, pos in garden_data: screen.blit(img, pos)
        my_dog.update(); my_dog.draw(screen)
        pygame.display.flip()
        clock.tick(60)
